...
/* Get information for peer indicated by peer_id */
...
/*
  Read through .map file and extract the peers as well as create the addr
  struct.
*/
void bt_parse_peer_list(bt_config_t *config) {
  ...
  while (fgets(line, BT_FILENAME_LEN, f) != NULL) {
    ...
    node = (bt_peer_t *) calloc(1, sizeof(bt_peer_t));
    assert(node != NULL);
