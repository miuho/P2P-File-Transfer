...
typedef struct bt_peer_s {
    mytime_t timer; /* records the transfer time of the peer */
    short  id;
    unsigned available, timeout_count;
    /* available returns if the peer is available for sending new GET packet */
    /* timeout_count records the number of times the peer has time-outed */
